...
[Посередник](../../mediator.html){.type} / [Go](../../go.html){.type}
:::
...
# **Посередник** на Go {#посередник-на-go .pattern-example-title-block-title}
::::
...
Система трафіку на залізничній платформі --- чудовий приклад патерну
Посередник. Два потяги ніколи не конфліктують між собою за наявність
вільної платформи, натомість менеджер станції `stationManager` виступає
посередником, який оголошує платформу вільною лише для одного потяга, а
решту тримає у черзі. Коли поїзд відправляється зі станції, він сповіщає
менеджера, що дає змогу прибути наступному в черзі поїзду.
...
#### []{#example-0--passengerTrain-go .anchor} **passengerTrain.go:** Конкретний компонент
...
#### []{#example-0--freightTrain-go .anchor} **freightTrain.go:** Конкретний компонент
...
#### []{#example-0--mediator-go .anchor} **mediator.go:** Інтерфейс посередника
...
#### []{#example-0--stationManager-go .anchor} **stationManager.go:** Конкретний посередник
