...
[Стратегія](../../strategy.html){.type} / [Java](../../java.html){.type}
:::
...
# **Стратегія** на Java {#стратегія-на-java .pattern-example-title-block-title}
::::
...
::: en-intro
 [Методи оплати в інтернет магазині](#example-0)
:::
...
**Застосування:** Стратегія часто використовується в Java-коді, особливо
там, де потрібно підміняти алгоритм під час виконання програми.
Починаючи з Java 8, багато прикладів стратегії можна замінити простими
lambda-виразами.
...
-   [`java.util.Comparator#compare()`](http://docs.oracle.com/javase/8/docs/api/java/util/Comparator.html#compare-T-T-),
    що викликаються з `Collections#sort()`.

-   [`javax.servlet.http.HttpServlet`](http://docs.oracle.com/javaee/7/api/javax/servlet/http/HttpServlet.html):
    метод `service()`, а також усі методи `doXXX()` приймають об'єкти
    `HttpServletRequest` та `HttpServletResponse` в параметрах.
...
## Методи оплати в інтернет магазині {#example-0-title}

У цьому прикладі Стратегія реалізує вибір платіжного методу в інтернет
магазині. Коли користувач сформував замовлення, він отримує вибір з
кількох платіжних засобів: електронного гаманця чи кредитної картки.

В даному випадку конкретні стратегії платіжних методів не тільки
проводять саму оплату, але й збирають необхідні дані на формі
замовлення.

### []{#example-0--strategies .anchor} **strategies** {#strategies .example-title}

#### []{#example-0--strategies-PayStrategy-java .anchor} **strategies/PayStrategy.java:** Загальний інтерфейс стратегій оплати
...
#### []{#example-0--strategies-PayByCreditCard-java .anchor} **strategies/PayByCreditCard.java:** Оплата кредиткою
...
#### []{#example-0--strategies-CreditCard-java .anchor} **strategies/CreditCard.java:** Кредитна картка
...
#### []{#example-0--order-Order-java .anchor} **order/Order.java:** Клас замовлення
