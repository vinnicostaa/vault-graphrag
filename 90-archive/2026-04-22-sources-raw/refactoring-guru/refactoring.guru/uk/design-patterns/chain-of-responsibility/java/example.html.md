...
# **Ланцюжок обов\'язків** на Java {#ланцюжок-обовязків-на-java .pattern-example-title-block-title}
::::
...
::: en-intro
 [Шари авторизації та аутентифікації користувачів](#example-0)
:::
...
**Застосування:** Патерн зустрічається в Java не так вже й часто,
оскільки для його застосування потрібен ланцюг об'єктів, наприклад,
зв'язаний список.

Сфера застосування ланцюжка обов'язків --- різноманітні обробники подій,
послідовні перевірки доступу та інше.

Приклади Ланцюжка обов'язків в стандартних бібліотеках Java:
...
## Шари авторизації та аутентифікації користувачів {#example-0-title}

Цей приклад показує як дані користувача проходять послідовну
аутентифікацію через багато обробників, зв'язаних в один ланцюг.

Цей приклад відрізняється від канонічної версії тим, що перевірка
припиняється, якщо черговий обробник **не може** обробити запит. В
класичному варіанті проходження ланцюжком закінчується відразу, як
тільки знаходиться елемент ланцюга, що **може** обробити запит. Просто
знайте, що Концептуальний приклад від цього не змінюється, а код
відрізняється тільки умовою виходу з ланцюга.

### []{#example-0--middleware .anchor} **middleware** {#middleware .example-title}

#### []{#example-0--middleware-Middleware-java .anchor} **middleware/Middleware.java:** Базовий клас перевірок
...
#### []{#example-0--middleware-ThrottlingMiddleware-java .anchor} **middleware/ThrottlingMiddleware.java:** Перевірка на ліміт запитів
...
#### []{#example-0--middleware-UserExistsMiddleware-java .anchor} **middleware/UserExistsMiddleware.java:** Перевірка пароля
...
#### []{#example-0--middleware-RoleCheckMiddleware-java .anchor} **middleware/RoleCheckMiddleware.java:** Перевірка ролі
...
#### []{#example-0--server-Server-java .anchor} **server/Server.java:** Сервер, на який заходимо
