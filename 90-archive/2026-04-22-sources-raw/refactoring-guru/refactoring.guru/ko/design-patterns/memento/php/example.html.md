...
[메멘토](../../memento.html){.type} / [PHP](../../php.html){.type}
:::
...
# PHP로 작성된 **메멘토** {#php로-작성된-메멘토 .pattern-example-title-block-title}
::::
...
**사용 사례들:** PHP에서 메멘토 패턴을 실제로 적용할 일이 있을지 매우
의심스럽습니다. 대부분의 경우 단순히 직렬화를 사용하여 객체 상태의
복사본을 더 쉽게 만들 수 있습니다.
::::::::::::::
...
이 예제에서는 PHP 웹 앱의 구성 관리 워크플로를 구현하기 위해 **메멘토**
패턴을 사용합니다. 많은 웹 앱은 사용자에게 설정을 맞춤 구성할 수 있는
기능을 제공하며, 사용자는 때때로 변경 사항을 되돌려 이전 구성을 복원하고
싶어 합니다. 변경이 일어나기 전에 구성 객체의 상태를 캡처하고 저장하면
메멘토 패턴으로 이러한 기능을 구현할 수 있습니다.
...
중재자](../../mediator/php/example.html){.btn .btn-default rel="prev"}
:::
