...
[메멘토](../../memento.html){.type} / [자바](../../java.html){.type}
:::
...
# 자바로 작성된 **메멘토** {#자바로-작성된-메멘토 .pattern-example-title-block-title}
::::
...
::: en-intro
 [모양 편집기와 복잡한 실행 취소/다시 실행](#example-0)
:::
...
이는 자바에서 매우 일반적입니다. 직렬화는 객체 상태의 스냅숏을 만드는
유일한 또는 가장 효율적인 방법은 아니나 다른 객체로부터 오리지네이터의
구조를 보호하면서 상태 백업을 저장할 수 있도록 합니다.
...
## 모양 편집기와 복잡한 실행 취소/다시 실행 {#example-0-title}

이 그래픽 편집기를 사용하면 화면에서 모양의 색상과 위치를 변경할 수
있으며, 또 모든 수정을 취소 또는 반복할 수 있습니다.

\'실행 취소\'는 메멘토와 커맨드 패턴 간의 협업을 기반으로 합니다.
편집기는 수행된 커맨드들의 기록을 추적합니다. 커맨드를 실행하기 전에
편집기는 백업을 만든 후 커맨드 객체에 연결합니다. 커맨드를 실행 후
편집기는 커맨드를 기록으로 푸시합니다.

사용자가 실행 취소를 요청하면 편집기는 기록에서 최근 명령을 가져온 후
해당 명령 내부에 보관된 백업에서 상태를 복원합니다. 사용자가 다른 실행
취소를 요청하면 편집기는 다음 명령을 기록에서 가져옵니다.

실행 취소된 명령들은 사용자가 화면의 모양들을 수정할 때까지 기록에
보관됩니다. 이것은 실행 취소된 명령을 다시 실행하는 데 중요합니다.

### []{#example-0--editor .anchor} **editor** {#editor .example-title}

#### []{#example-0--editor-Editor-java .anchor} **editor/Editor.java:** 편집자 코드
...
#### []{#example-0--editor-Canvas-java .anchor} **editor/Canvas.java:** 캔버스 코드
...
#### []{#example-0--history-History-java .anchor} **history/History.java:** 기록은 커맨드들과 메멘토들을 저장합니다
...
#### []{#example-0--history-Memento-java .anchor} **history/Memento.java:** 메멘토 클래스
...
#### []{#example-0--commands-Command-java .anchor} **commands/Command.java:** 기초 커맨드 클래스
...
#### []{#example-0--commands-ColorCommand-java .anchor} **commands/ColorCommand.java:** 선택된 모양의 색상 변경
...
#### []{#example-0--commands-MoveCommand-java .anchor} **commands/MoveCommand.java:** 선택된 모양을 이동합니다
...
### []{#example-0--shapes .anchor} **shapes:** 다양한 모양들 {#shapes-다양한-모양들 .example-title}
...
중재자](../../mediator/java/example.html){.btn .btn-default rel="prev"}
:::
