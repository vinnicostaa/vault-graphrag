...
[상태](../../state.html){.type} / [자바](../../java.html){.type}
:::
...
# 자바로 작성된 **상태** {#자바로-작성된-상태 .pattern-example-title-block-title}
::::
...
**사용 사례들:** 상태 패턴은 일반적으로 자바에서 대규모 `switch` 기반
상태 시스템을 객체로 변환하는 데 사용됩니다.

다음은 코어 자바 라이브러리로부터 가져온 상태 패턴의 몇 가지
예시들입니다:
...
-   [`javax.faces.lifecycle.LifeCycle#execute()`](http://docs.oracle.com/javaee/7/api/javax/faces/lifecycle/Lifecycle.html#execute-javax.faces.context.FacesContext-)
    ([`Faces­Servlet`](http://docs.oracle.com/javaee/7/api/javax/faces/webapp/FacesServlet.html)에
    의해 제어됨: 행동은 JSF 생명주기의 현재 단계​(상태)​에 의존함)
...
## 미디어 플레이어의 인터페이스 {#example-0-title}

이 예시에서 상태 패턴은 같은 미디어 플레이어 컨트롤들이 현재 재생 상태에
따라 다르게 작동하도록 합니다. 플레이어의 기본 클래스에는 상태 객체에
대한 참조가 포함되며, 이 객체는 플레이어를 위해 대부분 작업을
수행합니다. 일부 작업은 상태 객체를 다른 객체로 대처할 수 있으며, 이는
플레이어가 사용자 상호 작용에 반응하는 방식을 변경합니다.

### []{#example-0--states .anchor} **states** {#states .example-title}

#### []{#example-0--states-State-java .anchor} **states/State.java:** 공통 상태 인터페이스
...
#### []{#example-0--ui-Player-java .anchor} **ui/Player.java:** 플레이어의 기본 코드
...
#### []{#example-0--ui-UI-java .anchor} **ui/UI.java:** 플레이어의 그래픽 사용자 인터페이스
...
옵서버](../../observer/java/example.html){.btn .btn-default rel="prev"}
:::
