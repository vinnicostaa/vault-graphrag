...
[중재자](../../mediator.html){.type} / [러스트](../../rust.html){.type}
:::
...
# 러스트로 작성된 **중재자** {#러스트로-작성된-중재자 .pattern-example-title-block-title}
::::
...
It\'s not the only way to implement Mediator, but it\'s a fundamental
one.
...
반복자](../../iterator/rust/example.html){.btn .btn-default rel="prev"}
:::
