...
Type code affects the behavior of a class but we can\'t create
subclasses for the coded type due to the existing class hierarchy or
other reasons. Thus means that we can\'t apply [Replace Type Code with
Subclasses](replace-type-code-with-subclasses.html).

### Benefits

-   This refactoring technique is a way out of situations when a field
    with a coded type changes its value during the object\'s lifetime.
    In this case, replacement of the value is made via replacement of
    the state object to which the original class refers.
...
:::: dl
::: dt
[상태](design-patterns/state.html)
:::
::::

:::: dl
::: dt
[전략](design-patterns/strategy.html)
:::
::::
:::::::
