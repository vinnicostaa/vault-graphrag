...
Variable](split-temporary-variable.html), but in this case we\'re
dealing with a parameter, not a local variable.
...
and leads to unfortunate effects. Even if parameters are usually passed
by value (and not by reference) in your programming language, this
coding quirk may alienate those who are unaccustomed to it.
