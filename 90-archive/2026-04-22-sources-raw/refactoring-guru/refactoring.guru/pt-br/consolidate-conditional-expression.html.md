...
Before refactoring, make sure that the conditionals don't have any "side
effects" or otherwise modify something, instead of simply returning
values. Side effects may be hiding in the code executed inside the
operator itself, such as when something is added to a variable based on
the results of a conditional.
...
2.  Perform [Extract Method](extract-method.html) on the operator
    conditions and give the method a name that reflects the expression's
    purpose.
