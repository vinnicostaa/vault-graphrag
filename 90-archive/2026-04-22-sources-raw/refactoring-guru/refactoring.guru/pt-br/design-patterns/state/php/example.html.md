...
# **State** em PHP {#state-em-php .pattern-example-title-block-title}
::::
...
**Exemplos de uso:** O padrão State é ocasionalmente usado no PHP para
transformar máquinas de estado grandes e complicadas com base nos
operadores `switch` em objetos.
...
Neste exemplo, o padrão **State** é usado para representar vários
estados da Fatura. Esta abordagem permite implementar várias
verificações de condições ao fazer a transição da fatura de um estado
para outro, bem como encapsular a lógica de cada estado em uma classe
separada.

#### []{#example-1--index-php .anchor} **index.php:** Exemplo conceitual
