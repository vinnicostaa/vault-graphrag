...
# **Visitor** em PHP {#visitor-em-php .pattern-example-title-block-title}
::::
...
**Exemplos de uso:** O Visitor não é um padrão muito comum no PHP devido
à sua complexidade e aplicabilidade limitada.
::::::::::::::
...
Neste exemplo, o padrão **Visitor** ajuda a introduzir um recurso de
relatório em uma hierarquia de classes existente:
`Company (Empresa) > Department (Departamento) > Employee (Empregado)`

Depois que a infraestrutura do Visitor é adicionada à aplicação, você
pode facilmente adicionar outros comportamentos semelhantes à aplicação,
...
[Padrões de Projeto em Python []{.fa
