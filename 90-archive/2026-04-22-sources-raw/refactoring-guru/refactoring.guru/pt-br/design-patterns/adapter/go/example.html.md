...
# **Adapter** em Go {#adapter-em-go .pattern-example-title-block-title}
::::
...
Temos um código cliente que espera alguns recursos de um objeto
(Lightning port), mas temos outro objeto chamado *adaptee* (laptop
Windows) que oferece a mesma funcionalidade, mas por meio de uma
interface diferente (porta USB)

É aqui que o padrão Adapter entra em cena. Criamos um tipo de struct
conhecido como *adapter* que irá:

-   Seguir a mesma interface que o cliente espera (Lightning port).

-   Traduzir a solicitação do cliente para o adapter na forma que o
    adapter espera. O adapter aceita um conector Lightning e, em
    seguida, converte seus sinais em um formato USB e os passa para a
    porta USB no laptop com Windows.
...
#### []{#example-0--computer-go .anchor} **computer.go:** Interface cliente
...
#### []{#example-0--mac-go .anchor} **mac.go:** Serviço
...
#### []{#example-0--windows-go .anchor} **windows.go:** Serviço desconhecido
