...
# **Strategy** em Java {#strategy-em-java .pattern-example-title-block-title}
::::
...
::: en-intro
 [Método de pagamento em uma aplicação de e-commerce](#example-0)
:::
...
**Exemplos de uso:** O padrão Strategy é muito comum no código Java. É
...
O Java 8 trouxe o suporte de funções lambda, que podem servir como
alternativas mais simples ao padrão Strategy.

Aqui estão alguns exemplos do padrão Strategy nas principais bibliotecas
Java:

-   [`java.util.Comparator#compare()`](http://docs.oracle.com/javase/8/docs/api/java/util/Comparator.html#compare-T-T-)
    chamado a partir do `Collections#sort()`.

-   [`javax.servlet.http.HttpServlet`](http://docs.oracle.com/javaee/7/api/javax/servlet/http/HttpServlet.html):
    método `service()`, mais todos os métodos `doXXX()` que aceitam
    objetos `HttpServletRequest` e `HttpServletResponse` como
    argumentos.
...
## Método de pagamento em uma aplicação de e-commerce {#example-0-title}

Neste exemplo, o padrão Strategy é usado para implementar os vários
...
pagamento: Paypal ou cartão de crédito.

Strategies concretos não apenas executam o pagamento real, mas também
alteram o comportamento do formulário de pagamento, fornecendo campos
apropriados para registrar os detalhes do pagamento.

### []{#example-0--strategies .anchor} **strategies** {#strategies .example-title}

#### []{#example-0--strategies-PayStrategy-java .anchor} **strategies/PayStrategy.java:** Interface comum de métodos de pagamento
...
#### []{#example-0--strategies-PayByPayPal-java .anchor} **strategies/PayByPayPal.java:** Pagamento via PayPal
...
#### []{#example-0--strategies-PayByCreditCard-java .anchor} **strategies/PayByCreditCard.java:** Pagamento via cartão de crédito
...
#### []{#example-0--strategies-CreditCard-java .anchor} **strategies/CreditCard.java:** Uma classe de cartão de crédito
...
#### []{#example-0--order-Order-java .anchor} **order/Order.java:** Classe do pedido
