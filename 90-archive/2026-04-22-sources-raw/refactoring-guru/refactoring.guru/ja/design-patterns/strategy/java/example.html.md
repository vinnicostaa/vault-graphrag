...
# **Strategy** を Java で {#strategy-を-java-で .pattern-example-title-block-title}
::::
...
Java 8 で導入されたラムダ関数は[、]{.chpule2}[ ]{.chpuri2}Strategy
パターンの単純な代替として使えます[。]{.chpule2}[ ]{.chpuri2}

Java のコア・ライブラリーでの Strategy の使用例です[：]{.chpule2}[
]{.chpuri2}

-   [`java.util.Comparator#compare()`](http://docs.oracle.com/javase/8/docs/api/java/util/Comparator.html#compare-T-T-)
    は[、]{.chpule2}[ ]{.chpuri2}`Collections#sort()`
    から呼ばれます[。]{.chpule2}[ ]{.chpuri2}

-   [`javax.servlet.http.HttpServlet`](http://docs.oracle.com/javaee/7/api/javax/servlet/http/HttpServlet.html)[：]{.chpule2}[
    ]{.chpuri2}`service­()` メソッドと[、]{.chpule2}[
    ]{.chpuri2}`Http­Servlet­Request` と `Http­Servlet­Response`
    オブジェクトを引数として取る `do­XXX()` メソッドすべて[。]{.chpule2}[
    ]{.chpuri2}
...
## 電子商取引アプリでの支払い方法 {#example-0-title}

この例では[、]{.chpule2}[ ]{.chpuri2}Strategy
パターンを使用して[、]{.chpule2}[
]{.chpuri2}電子商取引プリケーションでさまざまな支払い方法を実装します[。]{.chpule2}[
]{.chpuri2}購入する商品を選択した後[、]{.chpule2}[ ]{.chpuri2}顧客は
Paypal
またはクレジットカードのいずれかの支払い方法を選択します[。]{.chpule2}[
]{.chpuri2}

具象ストラテジーは[、]{.chpule2}[
]{.chpuri2}単に実際の支払いを実行するだけではなく[、]{.chpule2}[
]{.chpuri2}チェックアウト用フォームの動作を変更して[、]{.chpule2}[
]{.chpuri2}支払いの詳細を記録するための適切なフィールドが存在するようにします[。]{.chpule2}[
]{.chpuri2}

### []{#example-0--strategies .anchor} **strategies** {#strategies .example-title}

#### []{#example-0--strategies-PayStrategy-java .anchor} **strategies/PayStrategy.java:** 支払いメソッドの共用インターフェース
...
#### []{#example-0--strategies-PayByPayPal-java .anchor} **strategies/PayByPayPal.java:** PayPal での支払い
...
#### []{#example-0--strategies-PayByCreditCard-java .anchor} **strategies/PayByCreditCard.java:** クレジットカードでの支払い
...
#### []{#example-0--strategies-CreditCard-java .anchor} **strategies/CreditCard.java:** クレジットカード・クラス
...
#### []{#example-0--order-Order-java .anchor} **order/Order.java:** 注文クラス
...
[[]{.fa .fa-arrow-left} State を Java
で](../../state/java/example.html){.btn .btn-default rel="prev"}
:::
