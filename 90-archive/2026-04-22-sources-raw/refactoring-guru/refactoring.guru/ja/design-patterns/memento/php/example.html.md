...
# **Memento** を PHP で {#memento-を-php-で .pattern-example-title-block-title}
::::
...
**使用例[：]{.chpule2}[ ]{.chpuri2}** PHP に Memento
パターンが実際に適用可能かは疑わしいところです[。]{.chpule2}[
]{.chpuri2}ほとんどの場合[、]{.chpule2}[
]{.chpuri2}オブジェクトの状態のコピーの作成は[、]{.chpule2}[
]{.chpuri2}単にシリアライゼーションを使えば簡単にできます[。]{.chpule2}[
]{.chpuri2}
::::::::::::::
...
この例では[、]{.chpule2}[ ]{.chpuri2}PHP Web
アプリの設定管理ワークフローを実装するために **Memento**
パターンを使用します[。]{.chpule2}[ ]{.chpuri2}多くの Web
アプリはユーザーに設定のカスタマイズを許可しており[、]{.chpule2}[
]{.chpuri2}ユーザーはときに変更を元に戻して以前の構成に戻したくなります[。]{.chpule2}[
]{.chpuri2}変更が行われる前に設定オブジェクトの状態を取得して保存することで[、]{.chpule2}[
]{.chpuri2}メメント (Memento)
パターンでこの機能を実現できます[。]{.chpule2}[ ]{.chpuri2}
...
[[]{.fa .fa-arrow-left} Mediator を PHP
で](../../mediator/php/example.html){.btn .btn-default rel="prev"}
:::
