...
# **State** を PHP で {#state-を-php-で .pattern-example-title-block-title}
::::
...
**使用例[：]{.chpule2}[ ]{.chpuri2}** PHP では[、]{.chpule2}[
]{.chpuri2}State パターンは[、]{.chpule2}[ ]{.chpuri2}`switch`
演算子に基づく巨大で厄介な状態機械をオブジェクトに変換する時に時々使用されます[。]{.chpule2}[
]{.chpuri2}
...
パターンを使用して[、]{.chpule2}[
]{.chpuri2}請求書のさまざまな状態を表現します[。]{.chpule2}[
]{.chpuri2}このアプローチにより[、]{.chpule2}[
]{.chpuri2}請求書をある状態から別の状態に移行する際にさまざまな条件チェックを実装でき[、]{.chpule2}[
]{.chpuri2}また各状態のロジックを別々のクラスにカプセル化できます[。]{.chpule2}[
]{.chpuri2}

#### []{#example-1--index-php .anchor} **index.php:** 概念的な例
...
[[]{.fa .fa-arrow-left} Observer を PHP
で](../../observer/php/example.html){.btn .btn-default rel="prev"}
:::
