...
# **State** を Java で {#state-を-java-で .pattern-example-title-block-title}
::::
...
-   [`javax.faces.lifecycle.LifeCycle#execute()`](http://docs.oracle.com/javaee/7/api/javax/faces/lifecycle/Lifecycle.html#execute-javax.faces.context.FacesContext-)[
    ]{.chpule1}[（]{.chpuri1}[`Faces­Servlet`](http://docs.oracle.com/javaee/7/api/javax/faces/webapp/FacesServlet.html)
    により管理[。]{.chpule2}[ ]{.chpuri2}振る舞いは[、]{.chpule2}[
    ]{.chpuri2}JSF ライフサイクルの現フェーズ[
    ]{.chpule1}[（]{.chpuri1}状態[）]{.chpule2}[
    ]{.chpuri2}に依存[）]{.chpule2}[ ]{.chpuri2}
...
## メディア・プレーヤーのインターフェース {#example-0-title}

この例では[、]{.chpule2}[ ]{.chpuri2}State
パターンを利用して[、]{.chpule2}[
]{.chpuri2}同じメディア・プレーヤー・コントロールが[、]{.chpule2}[
]{.chpuri2}現在の再生状態に応じて異なる振る舞いをします[。]{.chpule2}[
]{.chpuri2}プレーヤーの主クラスは[、]{.chpule2}[
]{.chpuri2}実際の仕事のほとんどをする状態オブジェクトへの参照を保持しています[。]{.chpule2}[
]{.chpuri2}アクションの中には[、]{.chpule2}[
]{.chpuri2}状態オブジェクトを他のものと入れ替えるものがあります[。]{.chpule2}[
]{.chpuri2}これにより[、]{.chpule2}[
]{.chpuri2}プレーヤーがユーザーに対応する仕方が変化します[。]{.chpule2}[
]{.chpuri2}

### []{#example-0--states .anchor} **states** {#states .example-title}

#### []{#example-0--states-State-java .anchor} **states/State.java:** 共通の状態インターフェース
...
#### []{#example-0--ui-Player-java .anchor} **ui/Player.java:** プレーヤーの主クラス
...
#### []{#example-0--ui-UI-java .anchor} **ui/UI.java:** プレーヤーの GUI
...
[[]{.fa .fa-arrow-left} Observer を Java
で](../../observer/java/example.html){.btn .btn-default rel="prev"}
:::
