...
[État](../../state.html){.type} / [PHP](../../php.html){.type}
:::
...
# **État** en PHP {#état-en-php .pattern-example-title-block-title}
::::
...
**Exemples d'utilisation :** L'état est parfois utilisé en PHP pour
transformer des automates finis trop encombrants reposant sur des
opérateurs `switch`, en objets.
...
Dans cet exemple, le patron **État** est utilisé pour représenter
différents états de la Facture. Cette approche permet d'implémenter
diverses vérifications de conditions lors de la transition de la facture
d'un état à un autre, ainsi que d'encapsuler la logique de chaque état
dans une classe séparée.

#### []{#example-1--index-php .anchor} **index.php:** Exemple conceptuel
