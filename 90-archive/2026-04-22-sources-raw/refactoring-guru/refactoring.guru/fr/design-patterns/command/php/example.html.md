...
[Commande](../../command.html){.type} / [PHP](../../php.html){.type}
:::
...
# **Commande** en PHP {#commande-en-php .pattern-example-title-block-title}
::::
...
**Exemples d'utilisation :** La commande est très répandue en PHP. Elle
est utilisée pour mettre les tâches dans une file d'attente, garder la
trace des tâches exécutées dans l'historique et effectuer l'action
« annuler ».
...
Dans cet exemple, la **Commande** est utilisée pour placer dans une file
d'attente des appels de web scraping (extraction du contenu d'un site)
dans le site Web IMDB, et les lancer un par un. La file d'attente est
sauvegardée dans une base de données afin de conserver les commandes
entre le lancement des scripts.
