...
[Médiateur](../../mediator.html){.type} / [Java](../../java.html){.type}
:::
...
# **Médiateur** en Java {#médiateur-en-java .pattern-example-title-block-title}
::::
...
::: en-intro
 [Application bloc-notes](#example-0)
:::
...
Java pour faciliter les communications entre les composants de
l'interface graphique d'une application. La partie contrôleur du modèle
MVC est un synonyme du médiateur.
...
-   [`java.util.Timer`](http://docs.oracle.com/javase/8/docs/api/java/util/Timer.html)
    (toutes les méthodes de `scheduleXXX()`)
...
-   [`java.util.concurrent.ExecutorService`](http://docs.oracle.com/javase/8/docs/api/java/util/concurrent/ExecutorService.html)
    (les méthodes `invokeXXX()` et `submit()`)
...
## Application bloc-notes {#example-0-title}

Dans cet exemple, nous allons voir comment organiser les nombreux
éléments d'une interface graphique et faire en sorte qu'ils coopèrent
sans être interdépendants à l'aide d'un médiateur.

### []{#example-0--components .anchor} **components:** Classes Collègue {#components-classes-collègue .example-title}
...
#### []{#example-0--mediator-Mediator-java .anchor} **mediator/Mediator.java:** Définit une interface Médiateur commune
...
#### []{#example-0--mediator-Editor-java .anchor} **mediator/Editor.java:** Médiateur concret
...
#### []{#example-0--mediator-Note-java .anchor} **mediator/Note.java:** Une classe Bloc-notes
