...
[Monteur](../../builder.html){.type} / [PHP](../../php.html){.type}
:::
...
# **Monteur** en PHP {#monteur-en-php .pattern-example-title-block-title}
::::
...
PHP. Il se montre très utile lorsque vous devez créer un objet possédant
...
Le monteur de requêtes SQL est l'un des meilleurs cas d'utilisation du
**Monteur**. L'interface du monteur définit les étapes communes
nécessaires au montage d'une requête SQL générique. De leur côté, les
monteurs concrets qui correspondent à différents dialectes SQL
implémentent ces étapes en retournant les morceaux des requêtes SQL qui
peuvent être exécutées sur un moteur de base de données particulier.
