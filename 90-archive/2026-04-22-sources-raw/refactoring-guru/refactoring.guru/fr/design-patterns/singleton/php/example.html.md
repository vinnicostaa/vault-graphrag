...
Le **Singleton** est bien connu pour limiter la réutilisation du code et
rendre les tests unitaires plus compliqués, mais il reste très utile
dans certains cas. Il est particulièrement pratique dans le cas où vous
devez contrôler des ressources partagées. Par exemple, pour un objet
d'historisation de logs qui doit contrôler l'accès à un fichier de logs.
Un autre bon exemple : le stockage de la configuration d'exécution
partagée.
