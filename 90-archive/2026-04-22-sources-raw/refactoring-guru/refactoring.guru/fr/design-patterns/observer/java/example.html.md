...
# **Observateur** en Java {#observateur-en-java .pattern-example-title-block-title}
::::
...
::: en-intro
 [Inscription aux événements](#example-0)
:::
...
**Exemples d'utilisation :** L'observateur est assez répandu en Java,
...
-   [`java.util.Observer`](http://docs.oracle.com/javase/8/docs/api/java/util/Observer.html)/[`java.util.Observable`](http://docs.oracle.com/javase/8/docs/api/java/util/Observable.html)
    (rarement utilisé dans le monde réel)
-   Toutes les implémentations de
    [`java.util.EventListener`](http://docs.oracle.com/javase/8/docs/api/java/util/EventListener.html)
    (pratiquement partout dans les composants Swing)
...
## Inscription aux événements {#example-0-title}

Dans cet exemple, l'observateur établit une collaboration indirecte
entre les objets d'un éditeur de texte. Chaque fois que l'objet
`Éditeur` est modifié, il envoie une notification à ses souscripteurs.
`EmailNotificationListener` et `LogOpenListener` réagissent à ces
notifications en exécutant leurs comportements principaux.

Les classes du souscripteur ne sont pas couplées aux classes de
l'éditeur et peuvent être réutilisées dans d'autres applications si
besoin. La classe `Éditeur` dépend uniquement de l'interface abstraite
du souscripteur. Ainsi nous pouvons ajouter de nouveaux types de
souscripteurs sans modifier le code de l'éditeur.

### []{#example-0--publisher .anchor} **publisher** {#publisher .example-title}

#### []{#example-0--publisher-EventManager-java .anchor} **publisher/EventManager.java:** Diffuseur de base
...
#### []{#example-0--editor-Editor-java .anchor} **editor/Editor.java:** Diffuseur concret suivi par d'autres objets
...
#### []{#example-0--listeners-EventListener-java .anchor} **listeners/EventListener.java:** Interface Observateur commune
...
#### []{#example-0--listeners-EmailNotificationListener-java .anchor} **listeners/EmailNotificationListener.java:** Envoie des e-mails lorsqu'il reçoit une notification
...
#### []{#example-0--listeners-LogOpenListener-java .anchor} **listeners/LogOpenListener.java:** Écrit un message dans une log lorsqu'il reçoit une notification
