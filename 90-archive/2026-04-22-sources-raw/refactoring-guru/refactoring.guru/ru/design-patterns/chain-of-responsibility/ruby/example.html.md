...
# **Цепочка обязанностей** на Ruby {#цепочка-обязанностей-на-ruby .pattern-example-title-block-title}
::::
...
**Применимость:** Паттерн встречается в Ruby не так уж часто, так как
...
<figure class="code">
<pre class="code" lang="ruby"><code># Интерфейс Обработчика объявляет метод построения цепочки обработчиков. Он
