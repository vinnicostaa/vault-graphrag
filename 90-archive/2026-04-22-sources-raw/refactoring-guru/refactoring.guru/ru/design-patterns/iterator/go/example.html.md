...
[Итератор](../../iterator.html){.type} / [Go](../../go.html){.type}
:::
...
# **Итератор** на Go {#итератор-на-go .pattern-example-title-block-title}
::::
...
Суть паттерна Итератор -- извлечь логику последовательного прохождения
коллекции в отдельный объект под названием «итератор». Этот итератор
предоставляет универсальные методы для последовательного прохождения
коллекции независимо от ее типа.

#### []{#example-0--collection-go .anchor} **collection.go:** Коллекция
...
#### []{#example-0--userCollection-go .anchor} **userCollection.go:** Конкретная коллекция
...
#### []{#example-0--iterator-go .anchor} **iterator.go:** Итератор
...
#### []{#example-0--userIterator-go .anchor} **userIterator.go:** Конкретный итератор
...
#### []{#example-0--user-go .anchor} **user.go:** Клиентский код
