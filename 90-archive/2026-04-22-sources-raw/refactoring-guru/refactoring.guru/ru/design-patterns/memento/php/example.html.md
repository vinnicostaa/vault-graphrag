...
[Снимок](../../memento.html){.type} / [PHP](../../php.html){.type}
:::
...
# **Снимок** на PHP {#снимок-на-php .pattern-example-title-block-title}
::::
...
**Применимость:** Реальная применимость паттерна Снимок в мире PHP под
большим вопросом. Чаще всего задачу хранения копии состояния можно
решить куда проще при помощи сериализации.
::::::::::::::
...
В этом примере паттерн **Снимок** используется для реализации процесса
управления конфигурацией PHP‑веб‑приложения. Многие веб‑приложения
позволяют пользователям настраивать параметры под себя, и иногда им
требуется откатить изменения и вернуть предыдущую конфигурацию. Паттерн
Снимок помогает реализовать такую возможность, фиксируя и сохраняя
состояние объекта конфигурации до внесения любых правок.
