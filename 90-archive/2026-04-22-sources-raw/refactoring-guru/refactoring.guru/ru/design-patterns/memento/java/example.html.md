...
[Снимок](../../memento.html){.type} / [Java](../../java.html){.type}
:::
...
# **Снимок** на Java {#снимок-на-java .pattern-example-title-block-title}
::::
...
::: en-intro
 [Отмена в редакторе фигур](#example-0)
:::
...
**Применимость:** Снимок на Java чаще всего реализуют с помощью
...
-   Все реализации
    ...
    могут быть использованы как аналог Снимка.
-   Все реализации
...
## Отмена в редакторе фигур {#example-0-title}

В данном графическом редакторе мы можем менять цвет и позицию фигур.
Пользователь может отменять или повторять действия, используя горячие
клавиши.

Механизм отмены построен при помощи паттерна Снимок и Команды. Перед
совершением команды, редактор сохраняет в истории снимок своего
состояния. Если нужно отменить команду, редактор берет последнюю запись
из истории и восстанавливает состояние из снимка. Для последующей отмены
берется команда дальше из истории.

Отмененные команды хранятся в истории пока пользователь не совершит
какое-то действие. Это нужно, чтобы иметь возможность повторять
отменённые команды.
...
#### []{#example-0--editor-Canvas-java .anchor} **editor/Canvas.java:** Холст
...
#### []{#example-0--history-History-java .anchor} **history/History.java:** История использует снимки и команды
...
#### []{#example-0--history-Memento-java .anchor} **history/Memento.java:** Снимок
...
#### []{#example-0--commands-Command-java .anchor} **commands/Command.java:** Интерфейс команд
...
#### []{#example-0--commands-ColorCommand-java .anchor} **commands/ColorCommand.java:** Команда смены цвета
...
#### []{#example-0--commands-MoveCommand-java .anchor} **commands/MoveCommand.java:** Команда перемещения
...
### []{#example-0--shapes .anchor} **shapes:** Разнообразные фигуры {#shapes-разнообразные-фигуры .example-title}
