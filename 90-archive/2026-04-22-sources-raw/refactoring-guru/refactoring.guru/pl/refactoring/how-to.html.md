...
makes the existing code slightly better while still leaving the
...
In this case, it's worthwhile to think about completely rewriting
parts of the code. But before that, you should have written tests and
set aside a good chunk of time. Otherwise, you'll end up with the
kinds of results we talked about in the first paragraph.
...
-   **You made an error during refactoring.** This one is a
    no-brainer: go ahead and fix the error.
...
-   **Your tests were too low-level.** For example, you were testing
    ...
    tests themselves or write an entirely new set of higher-level
    tests. A great way to avoid this kind of a situation is to write
