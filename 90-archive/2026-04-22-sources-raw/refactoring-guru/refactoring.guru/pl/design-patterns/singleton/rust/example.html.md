...
# **Singleton** w języku Rust {#singleton-w-języku-rust .pattern-example-title-block-title}
::::
...
arguments. The oldest living variable is an object created at the
start of the `main()`.
...
declaring a static variable with lazy initialization  at first
access. It is actually implemented via `unsafe` with `static mut`
