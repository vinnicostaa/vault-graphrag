...
# **Singleton** w języku PHP {#singleton-w-języku-php .pattern-example-title-block-title}
::::
...
antywzorzec, przez to jego wykorzystanie w kodzie PHP maleje.
...
Wzorzec **Singleton** cieszy się niesławą wskutek ograniczania ponownego
użycia kodu oraz komplikowania testów jednostkowych. Jest jednak bardzo
użyteczny w pewnych przypadkach --- na przykład gdy trzeba zapewnić
kontrolę dostępu do jakiegoś współdzielonego zasobu. Może to być
globalny obiekt dziennika nadzorujący dostęp do pliku dziennika, albo
współdzielony obszar przechowujący konfigurację programu w trakcie jego
działania.
