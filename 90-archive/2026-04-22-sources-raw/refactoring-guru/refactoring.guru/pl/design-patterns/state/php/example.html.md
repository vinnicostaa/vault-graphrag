...
[Stan](../../state.html){.type} / [PHP](../../php.html){.type}
:::
...
# **Stan** w języku PHP {#stan-w-języku-php .pattern-example-title-block-title}
::::
...
**Przykłady użycia:** Wzorzec Stan jest czasem stosowany w PHP w celu
przekształcania obszernych i skomplikowanych maszyn stanów opartych o
instrukcję `switch` w obiekty.
...
W tym przykładzie wzorzec **Stan** jest używany do reprezentowania
różnych stanów Faktury. To podejście pozwala na implementację różnych
sprawdzeń warunków podczas przechodzenia faktury z jednego stanu do
drugiego, a także na enkapsulację logiki każdego stanu w oddzielnej
klasie.

#### []{#example-1--index-php .anchor} **index.php:** Przykład koncepcyjny
