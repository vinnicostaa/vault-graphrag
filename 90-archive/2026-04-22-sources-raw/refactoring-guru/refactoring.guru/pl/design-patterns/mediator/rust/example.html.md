...
# **Mediator** w języku Rust {#mediator-w-języku-rust .pattern-example-title-block-title}
::::
...
4.  `Mediator` trait for the interaction between components
    (`notify_about_arrival`, `notify_about_departure`) is not the
    same as its external API for receiving external events (`accept`,
