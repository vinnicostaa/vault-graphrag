...
[Most](../../bridge.html){.type} / [PHP](../../php.html){.type}
:::
...
# **Most** w języku PHP {#most-w-języku-php .pattern-example-title-block-title}
::::
...
W tym przykładzie hierarchia `Page` (ang. strona) pełni rolę
**Abstrakcji**, zaś hierarchia `Renderer` --- **Implementacji**. Obiekty
klasy `Page` tworzą strony internetowe określonego typu za pomocą
podstawowych elementów otrzymywanych od obiektu `Renderer`, który jest
związany z tą stroną. Skoro obie hierarchie klas są rozdzielone, można
dodać kolejną klasę `Renderer` bez konieczności modyfikowania
którejkolwiek z klas `Page` --- i odwrotnie.
