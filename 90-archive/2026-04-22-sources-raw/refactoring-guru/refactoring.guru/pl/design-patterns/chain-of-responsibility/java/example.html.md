...
# **Łańcuch zobowiązań** w języku Java {#łańcuch-zobowiązań-w-języku-java .pattern-example-title-block-title}
::::
...
programach napisanych w Javie, gdyż ma sens tylko w tym kodzie, w którym
występują łańcuchy obiektów.

Jeden z najpopularniejszych przypadków użycia tego wzorca to propagacja
zdarzeń w kierunku nadrzędnych elementów klas graficznego interfejsu
użytkownika. Inny przykład to sekwencyjne filtry dostępu.

Oto parę przykładów zastosowania wzorca Łańcuch zobowiązań w głównych
bibliotekach Java:
...
## Filtrowanie dostępu {#example-0-title}

Poniższy przykład pokazuje jak żądanie zawierające dane użytkownika
przechodzi wzdłuż łańcucha obiektów obsługujących z których każdy
odpowiada za inne zadanie: uwierzytelnianie, autoryzacja, walidacja.

Podany przykład nieco się różni od kanonicznej wersji opisu wzorca
innych autorów. Większość przykładów bazuje na poszukiwaniu
odpowiedniego obiektu obsługującego żądanie, uruchamianiu go i
opuszczeniu łańcucha. W naszym przypadku aktywowany jest każdy obiekt
obsługujący, aż napotkamy taki, który **nie obsługuje** żądania. Jest to
nadal ten sam wzorzec Łańcucha zobowiązań, ale przedstawia inną
kolejność.

### []{#example-0--middleware .anchor} **middleware** {#middleware .example-title}

#### []{#example-0--middleware-Middleware-java .anchor} **middleware/Middleware.java:** Podstawowy interfejs walidacji
...
#### []{#example-0--middleware-ThrottlingMiddleware-java .anchor} **middleware/ThrottlingMiddleware.java:** Sprawdzanie limitu ilości żądań
...
#### []{#example-0--middleware-UserExistsMiddleware-java .anchor} **middleware/UserExistsMiddleware.java:** Sprawdzanie poświadczeń użytkownika
...
#### []{#example-0--middleware-RoleCheckMiddleware-java .anchor} **middleware/RoleCheckMiddleware.java:** Sprawdzanie roli użytkownika
...
#### []{#example-0--server-Server-java .anchor} **server/Server.java:** Cel autoryzacji
