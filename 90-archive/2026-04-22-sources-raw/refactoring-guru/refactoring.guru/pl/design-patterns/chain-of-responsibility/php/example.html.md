...
# **Łańcuch zobowiązań** w języku PHP {#łańcuch-zobowiązań-w-języku-php .pattern-example-title-block-title}
::::
...
**Przykłady użycia:** Wzorzec Łańcuch zobowiązań nie jest zbyt
powszechny w PHP ponieważ zakłada obecność łańcuchów obiektów. Jednym z
najsłynniejszych przykładów użycia tego wzorca w PHP jest
\[Oprogramowanie pośredniczące żądań HTTP\] opisane w PSR-15.
...
Najbardziej znanym przykładem użycia wzorca **Łańcuch zobowiązań** w
świecie PHP jest oprogramowanie pośredniczące HTTP. Implementuje się je
w najpopularniejszych frameworkach PHP, a nawet jest ustandaryzowane w
ramach PSR-15.

Działa ono w następujący sposób: żądanie HTTP musi przejść przez stos
obiektów pośredniczących aby mogło być obsłużone przez aplikację.
Każdy z obiektów-pośredników może albo odmówić dalszego przetwarzania
żądania, albo przekazać je kolejnemu obiektowi. Gdy żądanie pomyślnie
pokona całą drogę --- przejmuje je główny obiekt przetwarzający
aplikacji.

Być może widzisz już, że to podejście jest nieco przeciwstawne do
pierwotnego celu wzorca. I w rzeczy samej --- w typowej implementacji
żądanie jest przekazywane wzdłuż łańcucha jeśli bieżący obiekt NIE
MOŻE go obsłużyć; w tym zaś przypadku oprogramowanie pośredniczące
przekazuje żądanie dalej gdy "sądzi" że aplikacja MOŻE się nim zająć.
Niemniej jednak, ponieważ obiekty-składniki oprogramowania
pośredniczącego są połączone w łańcuch, cała koncepcja jest nadal
zgodna z ideą wzorca Łańcuch zobowiązań.
