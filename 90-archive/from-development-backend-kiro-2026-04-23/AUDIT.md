...
- **Raiz**: Implementação atual usa `sync.Map[ip]→limiter` só. `steering/security.md` §"Rate Limiting" dita: **300 req/min por userId** em rotas protegidas; **60 req/min por IP** apenas em `/auth/*`. Risco prático: um IP corporativo NAT (escritório com 50 síndicos) compartilha quota, um usuário agressivo esgota para todos.
- **Fix proposto**: Adicionar segundo tier `userLimiters sync.Map[userID]→limiter` aplicado depois de `Authenticate` via middleware `RateLimitPerUser(userRpm, userBurst)`. Rotas `/api/v1/*` ganham os dois limites (IP + user); rotas `/auth/*` continuam só IP. Registrar em `cmd/api/main.go` após Authenticate.
- **Sprint alvo**: Sprint 10 — não bloqueia marco 1 (volume atual baixo); spec compliance para GA.
...
### 2026-04-22 — BeyondCorp Audit (14 invariantes)

**12/14 invariantes validados em revisão linha-a-linha:**

- ✅ **Never trust frontend** — todos os handlers validam server-side; `DecodeJSON` + guards.
- ✅ **Zero Trust** — `Authenticate` aplicado em todo `/api/v1/*` via `RegisterModules`.
- ✅ **1-device per account** — `SyncUserUseCase.Execute` chama `sessionRepository.InvalidatePreviousByUserID` com session_id atual (`sync_user_use_case.go:106`).
- ✅ **Cookie httpOnly + Secure + SameSite** — `oidc_handler.go:110-112,166-168,181-183,200-202` (4 pontos de cookie setting) todos com `HttpOnly: true, Secure: h.secureCookies, SameSite: SameSiteLaxMode`.
- ✅ **Least privilege credentials** — Zitadel JWT Profile (`oauth.DefaultAuthorization(ZITADEL_KEY_PATH)`), Stripe test em dev, service account keys no `.gitignore` (A-018).
- ✅ **Secrets nunca em repo** — `.gitignore` cobre `.env`, `zitadel-key*.json`, binários; gosec G101 = 0.
- ✅ **Middleware stack ordenado** — `Recovery → RequestID → Logger → CORS → ErrorHandler → RateLimiter → Authenticate → RequirePermission` conforme CLAUDE.md §"Architectural Invariants".
- ✅ **Zitadel JWT Profile** — `zitadel_provider.go` usa `WithInsecure` apenas para `ZITADEL_PORT != 443` (dev path); prod usa TLS.
- ✅ **ABAC engine com matriz explícita** — `abac_rules.go` + `abac_engine.go` + 400 casos PBT + 100+ casos table-driven.
- ✅ **Webhook signature antes de parse** — Stripe (`verifyStripeSignature`) + Mux (`verifyMuxSignature`) — smoke test manual confirmou `/webhooks/stripe` rejeita body com HMAC inválido (400 "invalid signature").
- ✅ **Device fingerprint via `X-Device-Fingerprint`** — lido em `authenticate.go:110` e passado em `SyncUserCommand{DeviceFingerprint}`.
- ✅ **Vulnerability scanning** — `gosec -severity=high` = 0 findings (314 files); `govulncheck` = 0 CVEs; 3 Dependabot vulns fechadas em `d410547` (OTel 1.40→1.43).

**Pendentes (A-023 + 1 novo):**

- 🔴 **A-023** — 1-device change sem audit log comparativo. Listado em Itens Abertos.
- 🔴 **A-039** — Rate limiting só por IP; não aplica per-userId como especificado em `steering/security.md` §"Rate Limiting" (300 req/min por userId em rotas protegidas). Risco: um IP corporativo compartilhado (NAT) esgota quota de todos os usuários da empresa. Impact medium. Fix: adicionar tier adicional em `rate_limiter.go` com chave `userID` (extraída após Authenticate). Sprint 10.
