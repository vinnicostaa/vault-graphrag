---
title: "mastersindico-content-analysis.md"
type: source
tags: [pdf, converted]
source: 50-projects/master-sindico/legacy-docs/mastersindico-content-analysis.md.pdf
converted: 2026-04-22
---

# mastersindico-content-analysis.md

> PDF convertido via pdftotext. Layout original perdido; texto preservado.

Master Sindico - Arquitetura de Solucoes & Plano de
Execucao
Analise Arquitetural para Migracao de Monolito → Servicos Desacoplados
Autor: Analise tecnica consolidada Data: 23/03/2026 Stack: TypeScript/Go, Drizzle ORM, Bun, Zitadel (IAM),
OpenSearch Modelo de entrega: Sprints de 14 dias Equipe: 1 full-stack/arquiteto + 1 dev mobile/web + 2 designers
...
1.3 Tabela de Bounded Contexts
 ...
 Context        Owner       DB Tables                        Depende de   Publica Events
                ...
                Zitadel                                                   UserAuthenticated,
 Identity                   Zitadel DB                       Nenhum
                (externo)                                                 UserCreated

                            plans, subscriptions,
                            transactions, invoices,                       PlanResolved,
                Nosso
 Subscription               payment_methods,                 Identity     SubscriptionExpired,
                codigo
                            permissions, plan_permissions,                QuotaExceeded
                            plan_quotas, feature_usages
 Context          Owner           DB Tables                          Depende de       Publica Events

                                  condominiums, units,                                CondominiumCreated,
                  Nosso                                              Identity,
 Condominium                      members, mandates,                                  MemberJoined,
                  codigo                                             Subscription
                                  syndic_companies                                    MandateEnded

                                  timeline_entries,
                                                                                      TimelinePublished,
                                  master_plan_actions, events,
                  Nosso                                              Condominium,     PlanActionUpdated,
 Governance                       notices, validations,
                  codigo                                             Subscription     EventCreated,
                                  evaluations,
                                                                                      NoticePublished
                                  ...
                                  connect_me_requests,
                                  service_executions,
                                                                     Identity,        ConnectMeCreated,
                  Nosso           technical_activities,
 Marketplace                                                         Subscription,    ExecutionRegistered,
                  codigo          technical_notices,
                                                                     Condominium      ExecutionValidated
                                  company_proﬁles,
                                  company_users, agency_links

                  Nosso           videos, video_views,               Identity,        VideoUploaded,
 Media
                  codigo          video_likes, video_comments        Subscription     VideoViewed

                  OpenSearch                                         Todos (consome
 Search                           OpenSearch indices                                  Nenhum (read-only)
                  (externo)                                          events)
...
Regra 1: Nenhum contexto faz query direta em tabelas de outro contexto.
   ...
   • Certo: Governance chama SubscriptionService.canUserPublish(userId) ou recebe PlanResolved via
     middleware
...
 Feature                                                Esforco (dias)    Valor     Quadrante        Sprint
 ...
 Company Proﬁle + Users                                 3-4               Alto      QUICK WIN        S4
 ...
 Push Notiﬁcations (FCM)                                2-3               Medio     QUICK WIN        S6
...
Por que Vertical Slices para dev solo:
 ...
 Demonstravel             So no ﬁm do sprint                             A cada slice

 Bug discovery            Tardio (semana 2)                              Imediato

 Paralelismo com Dev 2    Diﬁcil (ambos no mesmo layer)                  Facil (cada um em slice diferente)
...
4.3 Regras de Paralelismo
 ...
 Feature ﬂags                   Background track sempre atras de feature ﬂag ate estar pronta
...
4.4 Background Track por Sprint
 ...
 S5       Content + Search        Push notiﬁcation setup (FCM)
...
Caminho critico: Zitadel → Billing → ABAC → Governance → Dashboard Se qualquer um desses atrasar, TUDO atrasa.
...
Recomendacao para MVP: Tudo em TypeScript. Go entra pos-lancamento para extrair servicos que precisem de
performance.
...
6. CALENDARIO FINAL (8 Sprints)
 Sprint    Periodo           Foco             Dev 1 (Vinni)                    Dev 2                 Background
                                              ...
                                              Monorepo, Zitadel, DB,                                 OpenSearch
 S0        23/03→06/04       Fundacao                                          Flutter setup, OIDC
                                              Redis, Core                                            cluster

                                              Billing, ABAC, Onboarding,       Flutter auth +        OS index
 S1        07/04→20/04       Identity
                                              Trial                            onboarding            mappings

                             Institutional    Registry, Timeline, Master       Flutter home +
 S2        21/04→04/05                                                                               OS sync job
                             I                Plan                             timeline

                             Institutional    Events, Notices, Dashboard,      Flutter morador       Mux video
 S3        05/05→18/05
                             II               Compliance                       panel                 pipeline

                                              Connect Me, Service,
 S4        19/05→01/06       Commercial                                        Flutter empresa       Email templates
                                              Company Proﬁle

                                              Video+Mux, Player,               Flutter video +
 S5        02/06→15/06       Content                                                                 FCM setup
                                              OpenSearch queries               search

                                              Push, Email, PDF, Mandato,
 S6        16/06→29/06       Integracoes                                       Flutter paridade      PDF templates
                                              ESV

                                              Assembly assincrona, bugs,
 S7        30/06→13/07       Assembly                                          Flutter assembly      OpenTelemetry
                                              performance

                                              Testes E2E, security, deploy
 Buﬀer     14/07→08/08       QA                                                Flutter QA            Monitoring
                                              prod

Marco 20/07: 80-90% pronto Marco 08/08: Entrega ﬁnal
...
#    Decisao              Opcoes                                        Impacto
     ...
     Payment                                                            Adapter Layer isola, mas PIX/Boleto sao
1                         Stripe vs Mercado Pago
     Gateway                                                            diferenciais

                          Hard block (403) vs Soft block (warning +
2    Paywall                                                            Simpliﬁca se hard block
                          grace)

3    Zitadel hosting      Cloud (managed) vs Self-hosted (Docker)       Cloud para MVP = menos ops

4    Go no MVP?           TypeScript only vs TS + Go para sync jobs     TS only simpliﬁca, Go pos-lancamento

     Mobile MVP           Core-only (auth, timeline, eventos) vs Full
5                                                                       Core-only nao bloqueia Dev 2
     scope                parity
