---
title: "Requirements v6 — fonte original (cliente, 2026-04-13)"
type: source
tags: [source, master-sindico, client-material, requirements, fase-c-archived]
project: master-sindico
absorbed_in_phase_c: true
absorbed_date: 2026-04-25
status: archived-source
---

> **⚠️ FONTE ORIGINAL — SUBSTITUÍDO POR `04-requirements/functional/*` CANÔNICO**
>
> Este documento foi absorvido em **Fase C (2026-04-25)** pelos `04-requirements/functional/{identity,billing,institutional,commercial,content,assembly,cross-domain}.md`. Termos legados foram traduzidos para canônico vigente:
> - N1/N2/N3 → `plan_tier ∈ {trial, base, plus, pro}` (D-081/D-096/ADR-0032)
> - "Morador Pagante" → não existe (D-126); só `trial/base/plus/pro`
> - Mercado Pago → Stripe canônico (BIL-001..BIL-055)
> - Mandate aggregate → flags em Membership (D-129)
> - Vote aggregate → entity de AgendaItem (D-130)
> - Adjustment commercial → cross-domain (D-131)
> - Tenant kinds 4 → 2 (D-126)
>
> **Reqs ABSORVIDOS** (deltas aplicados a `04-requirements/functional/`): R12 (Master Plan enums), R22 (Closed Deal termo_empresa+termo_sindico), R23 (Service Execution canônico), R25 (Adendo cross-BC), R26 (Company Evaluation textos), R27 (Vizinhança 38 cat), R29 (markers já cobertos), R36 (Conflict of Interest), R39 (Contractual Compliance), R40 (LGPD Registry schema), R41 (Governance Score — D-103 sobrescreve com 2 scores), R42 (Dossier blocking compliance), R46 (Mandate End), R47 (Mandate Transfer), R48 (Assembly enriched), R52 (Proxy 100% digital), R53 (Supplier Analyzer), R57 (Voting presencial assistido), R94 (Context Confirmer), R96 (QR Code), R98.1 (LMS enums).
>
> **Reqs REJEITADOS** (não absorvidos por inconsistência com canônico): stack TypeScript/Fastify (legado), Erlang OTP, Mercado Pago, IP-blocking (D-135 substitui), proxy upload PDF físico, N1/N2/N3 abstrato, "Morador Pagante" como categoria.
>
> **Reqs DELEGADOS PARA FASE D** (telas/UX): S1-S31 (síndico), M1-M15 (morador), E1-E16 (empresa), MK1-MK8 (marketing), AS1-AS8 (assembleia), AD1-AD8 (admin), VS1-VS10 (vizinhança síndico), VM1-VM7 (vizinhança morador), VE1-VE6 (vizinhança empresa), VZ1-VZ6 (vizinhança parceiro), 15 telas LMS — ver [[../_pendencias-fase-d]].
>
> Este arquivo permanece como **fonte original preservada** para auditoria de absorção. Não editar — modificações vão pra `04-requirements/functional/` canônico.
...
| Jornada Vizinhança/Comércio Local | Defines if it's high-domain or mid-domain | Domain classification |
| LMS Detailing | Defines if Content is high-domain or needs split | Domain classification |
...
1. Receive pending documents (Matriz Funcional, Admin, Vizinhança, LMS)
...
- ✅ Total de 108 screens mapeados (70 original + 10 VS + 15 LMS + 6 VE + 6 VZ + 7 VM = 114 screens)
...
- **Pending Decisions Documented:** 9 critical decisions requiring client input (Decisions 1, 3, 5 resolved; Decision 11 added for Morador→Síndico interaction model)
...
- **Requirements Updated:** 8 (Req 1 device/IP limits, Req 18 profile content limits, Req 19 harassment button + quotas, Req 27.2 EXPANDIDO com 7 telas, Req 29 video limits + visibility rules, Req 48 inadimplência deadline, Req 51 fração ideal validation, Req 53 supplier distinction, Req 57 auto-save)
