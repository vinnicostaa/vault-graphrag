...
# **State** in PHP {#state-in-php .pattern-example-title-block-title}
::::
...
**Usage examples:** The State pattern is occasionally used in PHP for
turning large and cumbersome state machines based on `switch` operators
into objects.
...
In this example, the **State** pattern is used to represent various
states of the Invoice. This approach allows to implement various
condition checks when transitioning the invoice from one state to
another, as well as to encapsulate the logic of each state into a
separate class.

#### []{#example-1--index-php .anchor} **index.php:** Conceptual example
