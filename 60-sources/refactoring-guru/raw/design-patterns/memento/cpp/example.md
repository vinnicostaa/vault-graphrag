...
# **Memento** in C++ {#memento-in-c .pattern-example-title-block-title}
::::
...
serialization, which is quite common in C++. While it's not the only and
the most efficient way to make snapshots of an object's state, it still
allows storing state backups while protecting the originator's structure
from other objects.
:::::::::::
