...
# **Memento** in PHP {#memento-in-php .pattern-example-title-block-title}
::::
...
**Usage examples:** The actual applicability of the Memento pattern in
PHP is very questionable. In most cases, you could make a copy of an
object's state easier by simply using serialization.
::::::::::::::
...
In this example, the **Memento** pattern is used to implement
configuration management workflow for a PHP web app. Many web apps allow
users to customize their settings. Sometimes users may want to revert
their changes and restore the previous configuration. The Memento
pattern can be used to implement this functionality by capturing and
storing the state of the configuration object before any changes are
made.
